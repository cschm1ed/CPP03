...
void ScavTrap::guardGate( void ) {
    if (_ep <= 0) {
        std::cout << "ScavTrap " << _name << " does not have enough energy points to perform Gate guarding\n";
        return;
    } else if (_hp <= 0) {
        std::cout << "ScavTrap " << _name << " does not have enough hit points to perform Gate guarding\n";
        return;
    }
